...
iprocessdelete (const int ihandle, off_t trownumber)
{
	...
	vb_rtd->iserrno =
		ivbdatawrite (ihandle, (void *) vb_rtd->pcwritebuffer, 1, trownumber);
	if (vb_rtd->iserrno) {
		return -1;
	}
...
isdelete (int ihandle, VB_CHAR * pcrow)
{
	...
	if (psvbfptr->pskeydesc[0]->k_flags & ISDUPS) {
		...
		switch (iresult) {
		case 1:					   /* Exact match */
			iresult = iprocessdelete (ihandle, psvbfptr->pskeycurr[0]->trownode);
			break;
...
int
isdelrec (int ihandle, vbisam_rec_n trownumber)
{
	vb_rtd_t   *vb_rtd = VB_GET_RTD;
	struct DICTINFO *psvbptr;
	int         iresult = 0;

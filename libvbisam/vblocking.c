...
static int
ilockinsert (const int ihandle, const vbisam_off_t trownumber)
{
...
static int
ilockdelete (const int ihandle, const vbisam_off_t trownumber)
{
	struct DICTINFO	*psvbptr;
...
*/
	...
	psvbptr->iindexchanged = 0;
	if (ivblock (psvbptr->iindexhandle, (vbisam_off_t)0, tlength, ilockmode)) {
		iserrno = EFLOCKED;
		return -1;
	}
	psvbptr->iisdictlocked |= 0x01;
/* RXW
	if (unlikely(ispecial)) {
		if (tvblseek (psvbptr->iindexhandle, (vbisam_off_t)0, SEEK_SET)) {
...
*/
	if (ivbblockread (ihandle, 1, (vbisam_off_t) 1, cvbnodetmp)) {
		psvbptr->iisdictlocked = 0;
		ivbexit (ihandle);
		iserrno = EBADFILE;
		return -1;
...
ivbexit (const int ihandle)
{
	...
	vbisam_off_t		tlength, ttransnumber;
...
*/
	if (psvbptr->iisdictlocked & 0x02) {
		...
		memcpy ((void *)cvbnodetmp, (void *)&psvbptr->sdictnode,
			sizeof (struct DICTNODE));
		iresult = ivbblockwrite (ihandle, 1, (vbisam_off_t) 1, cvbnodetmp);
		if (iresult) {
			iserrno = EBADFILE;
		} else {
			iserrno = 0;
	...
	tlength = VB_OFFLEN_3F;
	if (ivblock (psvbptr->iindexhandle, (vbisam_off_t)0, tlength, VBUNLOCK)) {
		iserrno = errno;
		return -1;
	}
	psvbptr->iisdictlocked = 0;
...
*/
	...
	if (ivblock (psvbptr->iindexhandle, toffset, (vbisam_off_t)1, ilocktype)) {
		return EFLOCKED;
	}
	if (imode == 2) {
		if (ivbblockread (ihandle, 1, (vbisam_off_t) 1, cvbnodetmp)) {
			(void)ivblock (psvbptr->iindexhandle, toffset, (vbisam_off_t)1, VBUNLOCK);
			return EBADFILE;
		}
		memcpy ((void *)&psvbptr->sdictnode, (void *)cvbnodetmp,
			sizeof (struct DICTNODE));
...
int
ivbdatalock (const int ihandle, const int imode, const vbisam_off_t trownumber)
{
	struct DICTINFO	*psvbptr;
	struct VBLOCK	*pslock;
	vbisam_off_t		tlength = 1, toffset;
	int		iresult = 0;

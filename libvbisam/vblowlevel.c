...
vbisam_off_t
tvblseek (const int ihandle, vbisam_off_t toffset, const int iwhence)
{
...
int
ivbblockread (const int ihandle, const int iisindex, const vbisam_off_t tblocknumber, char *cbuffer)
{
	...
	toffset = (vbisam_off_t) ((tblocknumber - 1) * psvbptr->inodesize);
	if (iisindex) {
...
int
ivbblockwrite (const int ihandle, const int iisindex, const vbisam_off_t tblocknumber, const char *cbuffer)
{
	struct DICTINFO	*psvbptr;
	vbisam_off_t		toffset;
	ssize_t		tresult;
...
int
ivblock (const int ihandle, const vbisam_off_t toffset, const vbisam_off_t tlength, const int imode)
{
#ifdef	_WIN32
...
/* This will probably not work correctly - 64 bit */
	int		itype, iresult = -1;
	vbisam_off_t		soffset;
	vbisam_off_t		tempoffset;

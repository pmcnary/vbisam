...
static inline void
inl_stquad (vbisam_off_t tvalue, void *pclocation)
{
#ifndef	WORDS_BIGENDIAN
#if	ISAMMODE == 1
...
struct	VBKEY {
	...
	vbisam_off_t		trownode;	/* The row / node number */
	vbisam_off_t		tdupnumber;	/* The duplicate number (1st = 0) */
...
struct	VBTREE {
	...
	vbisam_off_t		tnodenumber;	/* The actual node */
	vbisam_off_t		ttransnumber;	/* Transaction number stamp */
...
struct	DICTINFO {
	...
	int	dspare1;
	vbisam_off_t	trownumber;	/* Which data row is "CURRENT" 0 if none */
	vbisam_off_t	tdupnumber;	/* Which duplicate number is "CURRENT" (0=First) */
	vbisam_off_t	trowstart;	/* ONLY set to nonzero by isstart() */
	vbisam_off_t	ttranslast;	/* Used to see whether to set iindexchanged */
	vbisam_off_t	tnrows;		/* Number of rows (0 IF EMPTY, 1 IF NOT) */
	vbisam_off_t	tvarlennode;	/* Node containing 1st varlen data */
	char	*cfilename;
...
VB_HIDDEN extern int	ivbtransdelete (const int ihandle, vbisam_off_t trownumber, int irowlength);
...
VB_HIDDEN extern int	ivbtransinsert (const int ihandle, const vbisam_off_t trownumber,
				int irowlength, char *pcrow);
VB_HIDDEN extern int	ivbtransrename (char *pcoldname, char *pcnewname);
VB_HIDDEN extern int	ivbtranssetunique (const int ihandle, const vbisam_off_t tuniqueid);
VB_HIDDEN extern int	ivbtransuniqueid (const int ihandle, const vbisam_off_t tuniqueid);
VB_HIDDEN extern int	ivbtransupdate (const int ihandle, const vbisam_off_t trownumber,
				const int ioldrowlen, const int inewrowlen,
				const char *pcrow);

/* iswrite.c */
VB_HIDDEN extern int	ivbwriterow (const int ihandle, char *pcrow,
				const vbisam_off_t trownumber);

/* vbdataio.c */
VB_HIDDEN extern int	ivbdataread (const int ihandle, char *pcbuffer,
				int *pideletedrow, const vbisam_off_t trownumber);
VB_HIDDEN extern int	ivbdatawrite (const int ihandle, char *pcbuffer,
				int ideletedrow, const vbisam_off_t trownumber);

/* vbindexio.c */
VB_HIDDEN extern vbisam_off_t	tvbnodecountgetnext (const int ihandle);
VB_HIDDEN extern int	ivbnodefree (const int ihandle, const vbisam_off_t tnodenumber);
VB_HIDDEN extern int	ivbdatafree (const int ihandle, const vbisam_off_t trownumber);
VB_HIDDEN extern vbisam_off_t	tvbnodeallocate (const int ihandle);
VB_HIDDEN extern vbisam_off_t	tvbdataallocate (const int ihandle);
VB_HIDDEN extern int	ivbforcedataallocate (const int ihandle, const vbisam_off_t trownumber);
...
VB_HIDDEN extern int	ivbkeysearch (const int ihandle, const int imode,
			      ...
			      unsigned char *pckeyvalue, vbisam_off_t tdupnumber);
VB_HIDDEN extern int	ivbkeylocaterow (const int ihandle, const int ikeynumber, vbisam_off_t trownumber);
...
VB_HIDDEN extern int	ivbkeyinsert (const int ihandle, struct VBTREE *pstree,
			      ...
			      vbisam_off_t trownode, vbisam_off_t tdupnumber,
...
VB_HIDDEN extern vbisam_off_t	tvblseek (const int ihandle, vbisam_off_t toffset, const int iwhence);
...
VB_HIDDEN extern int	ivbblockread (const int ihandle, const int iisindex,
				const vbisam_off_t tblocknumber, char *cbuffer);
VB_HIDDEN extern int	ivbblockwrite (const int ihandle, const int iisindex,
				const vbisam_off_t tblocknumber, const char *cbuffer);
VB_HIDDEN extern int	ivblock (const int ihandle, const vbisam_off_t toffset,
				const vbisam_off_t tlength, const int imode);
...
VB_HIDDEN extern int	ivbnodeload (const int ihandle, const int ikeynumber,
				struct VBTREE *pstree, const vbisam_off_t tnodenumber,
				const int iprevlvl);

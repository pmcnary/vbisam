...
ttailnode (const int ihandle, char *pcbuffer, const int ilength, int *pislotnumber)
{
	...
	vbisam_off_t		tnodenumber = 0, tnodenext, tnodeprev;
	...
	if (igroup != pshdr->cgroup) {
		...
		inl_stquad ((vbisam_off_t)0, pshdr->cfreeprev);
		tnodenext =
		    inl_ldquad (psvbptr->sdictnode.cvarleng0 + (igroup * QUADSIZE));
...
static int
ivbvarlenread (const int ihandle, char *pcbuffer, vbisam_off_t tnodenumber, int islotnumber, int ilength)
{
	char	*pcnodeptr;
...
{
	struct DICTINFO	*psvbptr;
	vbisam_off_t		tnewnode, tnodenumber = 0;
	int		islotnumber;
	int		inodesize;
	int		n;
	...
	while (ilength > 0) {
		if (ilength >
			...
			inl_stquad ((vbisam_off_t)0, psvarlenheader->cfreenext);
			inl_stquad ((vbisam_off_t)0, psvarlenheader->cfreeprev);
...
static int
ivbvarlendelete (const int ihandle, vbisam_off_t tnodenumber, int islotnumber, int ilength)
{
	struct DICTINFO	*psvbptr;
	vbisam_off_t		tnodenext, tnodeprev;
 ...
 */
int
ivbdataread (const int ihandle, char *pcbuffer, int *pideletedrow, const vbisam_off_t trownumber)
{
	struct DICTINFO *psvbptr;
	vbisam_off_t		tblocknumber, toffset, tsofar;
	int		irowlength;
	int		n;
...
int
ivbdatawrite (const int ihandle, char *pcbuffer, int ideletedrow, const vbisam_off_t trownumber)
{
	struct DICTINFO *psvbptr;
	char		*pctemp;
	vbisam_off_t		tblocknumber, toffset, tsofar;

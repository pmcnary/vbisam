...
#ifdef	WITH_LFS64
#define VB_MAX_OFF_T	(vbisam_off_t)9223372036854775807LL
#else
#define VB_MAX_OFF_T	(vbisam_off_t)2147483647
#endif
...
ivbkeysearch (const int ihandle, const int imode, const int ikeynumber,
	      int ilength, unsigned char *pckeyvalue, vbisam_off_t tdupnumber)
{
	struct VBKEY	*pskey;
	struct keydesc	*pskeydesc;
	int		iresult;
...
int
ivbkeylocaterow (const int ihandle, const int ikeynumber, vbisam_off_t trownumber)
{
	struct DICTINFO	*psvbptr;
	struct VBKEY	*pskey;
	struct VBTREE	*pstree;
...
ivbkeyinsert (const int ihandle, struct VBTREE *pstree, const int ikeynumber,
	      unsigned char *pckeyvalue, vbisam_off_t trownode, vbisam_off_t tdupnumber,

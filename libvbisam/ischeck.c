...
static vbisam_off_t	gtlastuseddata;		/*  Last row USED in data file */
static vbisam_off_t	gtdatasize;		/*  # Rows in data file */
static vbisam_off_t	gtindexsize;		/*  # Nodes in index file */
...
static int
ibittestandset (char *psmap, vbisam_off_t tbit)
{
	tbit--;
	switch (tbit % 8) {
	case 0:
...
static int
ibittestandreset (char *psmap, vbisam_off_t tbit)
{
	tbit--;
	switch (tbit % 8) {
	case 0:
...
ipreamble (ihandle)
{
	...
	if (psvbptr->iopenmode & ISVARLEN) {
		gtdatasize =
		    (vbisam_off_t) (sstat.st_size + psvbptr->iminrowlength + INTSIZE +
			     QUADSIZE) / (psvbptr->iminrowlength + INTSIZE +
					  QUADSIZE + 1);
	} else {
		gtdatasize = (vbisam_off_t) (sstat.st_size +
	...
	gtindexsize = (vbisam_off_t)(sstat.st_size + psvbptr->inodesize -
		     1) / psvbptr->inodesize;
...
{
	struct DICTINFO	*psvbptr;
	vbisam_off_t		tfreehead, tfreerow, tholdhead, tloop;
	int		iloop, iresult;
...
{
	struct DICTINFO	*psvbptr;
	vbisam_off_t		tfreehead, tfreenode, tholdhead;
	int		iloop, iresult;
...
static int
ichecktree (int ihandle, int ikey, vbisam_off_t tnode, int ilevel)
{
	int		iloop;
	struct VBTREE	stree;
...
static void
vaddkeyforrow (int ihandle, int ikey, vbisam_off_t trownumber)
{
	struct DICTINFO	*psvbptr;
	struct VBKEY	*psKey;
	vbisam_off_t		tdupnumber;
	int		iresult;

...
static int
irowupdate (const int ihandle, char *pcrow, vbisam_off_t trownumber)
{
	struct VBKEY	*pskey;
	struct DICTINFO	*psvbptr;
	struct keydesc	*pskeyptr;
	vbisam_off_t		tdupnumber = 0;
...
isrewrite (const int ihandle, char *pcrow)
{
	...
	if (psvbptr->pskeydesc[0]->k_flags & ISDUPS) {
		...
		iresult = ivbkeysearch (ihandle, ISEQUAL, 0, 0, ckeyvalue, (vbisam_off_t)0);
		switch (iresult) {
		case 1:	/* Exact match */
			iresult = 0;
			...
			if (!iresult) {
				...
				isreclen = inewreclen;
				iresult =
				    ivbdatawrite (ihandle, pcrow, 0, (vbisam_off_t)isrecnum);
			}
			if (!iresult) {
...
isrewcurr (const int ihandle, char *pcrow)
{
	...
	if (psvbptr->trownumber > 0) {
		...
		if (!iresult) {
			...
			isreclen = inewreclen;
			iresult = ivbdatawrite (ihandle, pcrow, 0, (vbisam_off_t)isrecnum);
		}
		if (!iresult) {
			if (psvbptr->iopenmode & ISVARLEN) {
				iresult =

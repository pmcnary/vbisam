...
iaddkeydescriptor (const int ihandle, struct keydesc *pskeydesc)
{
	...
	char		*pcdstptr;
	vbisam_off_t		theadnode, tnodenumber = 0, tnewnode;
	...
	inl_stquad ((vbisam_off_t)1, cvbnodetmp + INTSIZE);
#else
...
static int
idelnodes (const int ihandle, const int ikeynumber, vbisam_off_t trootnode)
{
	struct DICTINFO	*psvbptr;
	char		*pcsrcptr;
...
imakekeysfromdata (const int ihandle, const int ikeynumber)
{
	...
	for (tloop = 1; tloop < inl_ldquad (psvbptr->sdictnode.cdatacount); tloop++) {
		...
		iresult = ivbkeysearch (ihandle, ISGREAT, ikeynumber, 0, ckeyvalue, (vbisam_off_t)0);
		tdupnumber = 0;
...
isbuild (const char *pcfilename, const int imaxrowlength, struct keydesc *pskey, int imode)
{
	...
	inl_stquad ((vbisam_off_t)2, psvbptr->sdictnode.cnodekeydesc);
	inl_stquad ((vbisam_off_t)0, psvbptr->sdictnode.cdatafree);
	inl_stquad ((vbisam_off_t)0, psvbptr->sdictnode.cnodefree);
	inl_stquad ((vbisam_off_t)0, psvbptr->sdictnode.cdatacount);
	if (pskey->k_nparts) {
		inl_stquad (3, psvbptr->sdictnode.cnodecount);
	} else {
		inl_stquad (2, psvbptr->sdictnode.cnodecount);
	}
	inl_stquad ((vbisam_off_t)1, psvbptr->sdictnode.ctransnumber);
	inl_stquad ((vbisam_off_t)1, psvbptr->sdictnode.cuniqueid);
	inl_stquad ((vbisam_off_t)0, psvbptr->sdictnode.cnodeaudit);
	...
	if (ivbblockwrite (ihandle, 1, (vbisam_off_t) 1, cvbnodetmp)) {
		ivbclose (psvbptr->iindexhandle);	/* Ignore ret */
	...
	pctemp = cvbnodetmp;
	pctemp += INTSIZE;
	inl_stquad ((vbisam_off_t)0, pctemp);	/* Next keydesc node */
	pctemp += QUADSIZE;
	/* keydesc length */
	inl_stint (INTSIZE + QUADSIZE + 1 + (((INTSIZE * 2) + 1) * pskey->k_nparts), pctemp);
	pctemp += INTSIZE;
	if (pskey->k_nparts) {
		inl_stquad ((vbisam_off_t)3, pctemp);	/* Root node for this key */
	} else {
		inl_stquad ((vbisam_off_t)0, pctemp);	/* Root node for this key */
	}
	pctemp += QUADSIZE;
	*pctemp = pskey->k_flags / 2;	/* Compression / Dups flags */
	...
	if (ivbblockwrite (ihandle, 1, (vbisam_off_t) 2, cvbnodetmp)) {
		ivbclose (psvbptr->iindexhandle);	/* Ignore ret */
		ivbclose (psvbptr->idatahandle);	/* Ignore ret */
		if (psvbptr->cfilename) {
			free (psvbptr->cfilename);
...
#if	ISAMMODE == 1
		inl_stint (INTSIZE + QUADSIZE, cvbnodetmp);
		inl_stquad ((vbisam_off_t)1, cvbnodetmp + INTSIZE);	/* Transaction number */
#else
		inl_stint (INTSIZE, cvbnodetmp);
#endif
		if (ivbblockwrite (ihandle, 1, (vbisam_off_t) 3, cvbnodetmp)) {
			ivbclose (psvbptr->iindexhandle);	/* Ignore ret */
			ivbclose (psvbptr->idatahandle);	/* Ignore ret */
			if (psvbptr->cfilename) {
				free (psvbptr->cfilename);

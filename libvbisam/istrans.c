...
iwritetrans (int itranslength, const int irollback)
{
	...
	iresult = ivblock (ivblogfilehandle, (vbisam_off_t)0, (vbisam_off_t)1, VBWRLCKW);
	if (iresult) {
		return ELOGWRIT;
	}
	psvblogheader = (struct SLOGHDR *)cvbtransbuffer;
	if (irollback) {
		...
		toffset = tvblseek (ivblogfilehandle, (vbisam_off_t)0, SEEK_END);
		if (toffset == -1) {
			goto writerr;
		}
		...
		if (tvblseek (ivblogfilehandle, (vbisam_off_t)0, SEEK_END) == -1) {
			goto writerr;
		}
	}
	...
	iresult = ivblock (ivblogfilehandle, (vbisam_off_t)0, (vbisam_off_t)1, VBUNLOCK);
	if (iresult) {
		return ELOGWRIT;
	}
	...
	return 0;
writerr:
	(void)ivblock (ivblogfilehandle, (vbisam_off_t)0, (vbisam_off_t)1, VBUNLOCK);
	return ELOGWRIT;
}
...
idemotelocks (void)
{
	...
	for (ihandle = 0; ihandle <= ivbmaxusedhandle; ihandle++) {
		...
		if (ivbdatalock (ihandle, VBUNLOCK, (vbisam_off_t)0)) {	/* BUG Only ours? */
			iresult = -1;
		}
	}
	if (iresult) {
...
static int
ivbrollmeback (vbisam_off_t toffset, const int iinrecover)
{
	char	*pcbuffer, *pcrow;
	vbisam_off_t	tlength, trownumber;
	int	iloop;
	...
	while (!ifoundbegin) {
		...
		if (toffset == -(INTSIZE)) {
			if (tvblseek (ivblogfilehandle, (vbisam_off_t)0, SEEK_SET) != 0) {
				return EBADFILE;
			}
...
static int
ivbrollmeforward (vbisam_off_t toffset)
{
	char	*pcbuffer;
	vbisam_off_t	tlength, trownumber;
	int	ifoundbegin = 0;
...
int
ivbtransdelete (const int ihandle, vbisam_off_t trownumber, int irowlength)
{
	struct DICTINFO	*psvbptr;
	char		*pcbuffer;
...
int
ivbtransinsert (const int ihandle, const vbisam_off_t trownumber, int irowlength, char *pcrow)
{
	struct DICTINFO	*psvbptr;
	char		*pcbuffer;
...
int
ivbtranssetunique (const int ihandle, const vbisam_off_t tuniqueid)
{
	struct DICTINFO	*psvbptr;
	char		*pcbuffer;
...
int
ivbtransuniqueid (const int ihandle, const vbisam_off_t tuniqueid)
{
	struct DICTINFO	*psvbptr;
	char		*pcbuffer;
...
int
ivbtransupdate (const int ihandle, const vbisam_off_t trownumber, const int ioldrowlen, const int inewrowlen, const char *pcrow)
{
	struct DICTINFO	*psvbptr;
	char		*pcbuffer;
	int		ilength;

	...
	while (ideleted) {
		if (isrecnum > 0
			...
			if (psvbptr->iopenmode & ISAUTOLOCK || imode & ISLOCK) {
				if (ivbdatalock (ihandle, imode & ISWAIT ? VBWRLCKW : VBWRLOCK, (vbisam_off_t)isrecnum)) {
					ilockresult = ELOCKED;
					iresult = -1;
				}
			}
			if (!ilockresult) {
				iresult = ivbdataread (ihandle,
						 ...
						 &ideleted, (vbisam_off_t)isrecnum);
			}
			if (iresult) {
				isrecnum = 0;
				iserrno = EBADFILE;
...
isread (const int ihandle, char *pcrow, int imode)
{
	...
	case ISFIRST:
		...
		iresult = ivbkeysearch (ihandle, ISFIRST, ikeynumber, 0, ckeyvalue, (vbisam_off_t)0);
		if (iresult < 0) {
			break;
		}
		if (iresult == 2) {
	...
	case ISLAST:
		 ...
		 */
		iresult = ivbkeysearch (ihandle, ISLAST, ikeynumber, 0, ckeyvalue, (vbisam_off_t)0);
		if (iresult < 0) {
			break;
		}
		if (iresult == 2) {
	...
	case ISEQUAL:
		...
		iresult = ivbkeysearch (ihandle, ISGTEQ, ikeynumber, 0, ckeyvalue, (vbisam_off_t)0);
		if (iresult == -1) {	/* Error */
			break;
		}
	...
	case ISGTEQ:
		vvbmakekey (psvbptr->pskeydesc[ikeynumber], pcrow, ckeyvalue);
		iresult = ivbkeysearch (ihandle, ireadmode, ikeynumber, 0, ckeyvalue, (vbisam_off_t)0);
...
isstart (const int ihandle, struct keydesc *pskeydesc, int ilength, char *pcrow, int imode)
{
	...
	case ISEQUAL:
		...
		if (ilength < pskeydesc->k_len) {
			...
			iresult = ivbkeysearch (ihandle, ISEQUAL, ikeynumber, ilength, ckeyvalue, (vbisam_off_t)0);
		}
		iserrno = EBADFILE;
		if (iresult == -1) {	/* Error */
			break;
	...
	case ISGTEQ:
		...
		memcpy (ckeyvalue, ckeyvalue2, (size_t)ilength);
		iresult = ivbkeysearch (ihandle, imode, ikeynumber, 0, ckeyvalue, (vbisam_off_t)0);
		if (iserrno == EENDFILE) {
			iserrno = ENOREC;
			iresult = -1;
			break;

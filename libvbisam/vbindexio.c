...
int
ivbnodefree (const int ihandle, const vbisam_off_t tnodenumber)
{
	struct DICTINFO *psvbptr;
	...
	if (theadnode == 0) {
		...
		inl_stquad ((vbisam_off_t)0, cvbnodetmp2 + INTSIZE);
...
int
ivbdatafree (const int ihandle, const vbisam_off_t trownumber)
{
	struct DICTINFO *psvbptr;
	vbisam_off_t		theadnode, tnodenumber;
	int		ilengthused, iresult;
...
/* C-ISAM is not 100% C-ISAM compatible */
		...
		if (ilengthused < psvbptr->inodesize - (QUADSIZE + 3)) {
			...
			inl_stquad ((vbisam_off_t) trownumber, cvbnodetmp + ilengthused);
			ilengthused += QUADSIZE;
			inl_stint (ilengthused, cvbnodetmp);
			iresult = ivbblockwrite (ihandle, 1, theadnode, cvbnodetmp);
			return iresult;
...
{
	struct DICTINFO *psvbptr;
	vbisam_off_t		theadnode, tnextnode, tvalue;
	int		ilengthused, iresult;
...
/* C-ISAM is not 100% C-ISAM compatible */
		...
		if (ilengthused > INTSIZE + QUADSIZE) {
			...
			inl_stquad ((vbisam_off_t)0, &cvbnodetmp[ilengthused]);
			if (ilengthused > INTSIZE + QUADSIZE) {
				iserrno = ivbblockwrite (ihandle, 1, theadnode, cvbnodetmp);
				if (iserrno) {
					return -1;
...
int
ivbforcedataallocate (const int ihandle, const vbisam_off_t trownumber)
{
	struct DICTINFO *psvbptr;
	vbisam_off_t		theadnode, tprevnode, tnextnode;
